...
const API_URL = "https://fivelegflex-backend.fly.dev/api/best-props-cfb";
const LOCAL_URL = "http://0.0.0.0:8080/api/best-props-cfb";
    ...
    const fetchData = async () => {
      try {
        const res = await fetch(
          `${url}?include_prizepicks=${includePrizePicks}`,
          {
            method: "GET",
            headers: {
  ...
  return (
    <div className="flex flex-col md:flex-row justify-between items-center mb-6 space-y-2 md:space-y-0 md:space-x-4 w-full">
      <div className="flex flex-row space-x-2 mb-2 md:mb-0">
        <button
          className={`px-4 py-2 font-semibold rounded-lg transition-transform duration-300 ${
            selectedFilter === "all"
              ? "bg-gold text-fullblack border border-lightgray"
              : "bg-white border border-lightgray dark:bg-black dark:text-white hover:bg-opacity-80"
          } h-12 md:h-10`}
        ...
        <button
          className={`px-4 py-2 font-semibold rounded-lg transition-transform duration-300 ${
            selectedFilter === "over"
              ? "bg-green text-fullblack border border-lightgray"
              : "bg-white border border-lightgray dark:bg-black dark:text-white hover:bg-opacity-80"
          } h-12 md:h-10`}
        ...
        <button
          className={`px-4 py-2 font-semibold rounded-lg transition-transform duration-300 ${
            selectedFilter === "under"
              ? "bg-red text-fullblack border border-lightgray"
              : "bg-white border border-lightgray dark:bg-black dark:text-white hover:bg-opacity-80"
          } h-12 md:h-10`}
      ...
      <div className="flex items-center space-x-2">
        <label className="text-fullblack dark:text-white font-semibold">
          PrizePicks Only
        </label>
...
const CFBPropsPage = () => {
  ...
  const uniqueGames = [
    ...new Set(
      bettingProps?.data?.map(
        (prop) => `${prop.home_team} vs ${prop.away_team}`
      )
    ),
  ];
  ...
  const filteredProps = bettingProps?.data?.filter((prop) => {
    const filterMatch =
      selectedFilter === "all" || prop.bestBet === selectedFilter;
    const propTypeMatch =
      selectedPropType === "all" ||
      prop.prop_type === propTypeMapping[selectedPropType];
    const gameMatch =
      selectedGame === "all" ||
      `${prop.home_team} vs ${prop.away_team}` === selectedGame;
            ...
            <div className="flex justify-center items-center w-full">
              <div className="bg-offwhite dark:bg-fullblack text-fullblack dark:text-white p-4 md:p-6 lg:p-8">
                <h1 className="text-center text-4xl font-bold mb-4">
                  {bettingProps?.message || "No props available"}
                </h1>
              </div>
            </div>
          )}
